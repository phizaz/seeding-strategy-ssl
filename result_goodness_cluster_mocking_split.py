...
def run_and_save(dataset):
    ...
    def normal():
        return Pipe() \
            ...
            .split(len(seeding_fns), seeder(seeding_fns, seeding_names, name=dataset.name)) \
                ...
                .pipe(goodness_cluster_mocking_nested_split(method='average')) \
                .pipe(goodness_cluster_mocking_nested_split(method='complete')) \
                .pipe(goodness_cluster_mocking_nested_split(method='single')) \
            .merge('result', group('evaluation_kmeans_3',
                                   'label_correctness_kmeans_3',
                                   'goodness_cluster_mocking_nested_split_ward',
                                   'goodness_cluster_mocking_nested_split_average',
                                   'goodness_cluster_mocking_nested_split_complete',
                                   'goodness_cluster_mocking_nested_split_single',
                                   'name')) \
            .connect(stop())
    ...
    with open('results/goodness_cluster_mocking_split-' + dataset.name + '.json', 'w') as file:
